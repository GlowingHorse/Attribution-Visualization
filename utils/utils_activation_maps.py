...
def create_root_dir(img_name, attr_class, flag1, img_info_name='img_info', experiment_dir='./experiment/'):
  name_str = os.path.splitext(os.path.basename(img_name))[-2]
  root_directory = experiment_dir + img_info_name + '/' + name_str + '/' + attr_class + '_' + flag1
  if not os.path.exists(root_directory):
    os.makedirs(root_directory)
  return root_directory


def create_advsys_root_dir(img_name, attr_class):
  name_str = os.path.splitext(os.path.basename(img_name))[-2]
  root_directory = './experiment/advsys_results/vis/' + name_str + '/' + attr_class
  if not os.path.exists(root_directory):
    os.makedirs(root_directory)
  return root_directory


def create_decomposition_dir(root_directory, decomposition_method,
                              no_slash_layer_name, reverse_suffix,
                              n_groups):
  save_directory = root_directory + '/' + decomposition_method + '/' \
...
def decompose_AM_get_group_num(decomposition_method, AM, thres_explained_var, specific_group_num=None):
  if specific_group_num is not None:
    factor_model = MatrixDecomposer(specific_group_num, decomposition_method)
    ...
    n_groups = specific_group_num
    if len(spatial_factors.shape) == 2:
      pass
    else:
      spatial_factors = spatial_factors.transpose(2, 0, 1).astype("float32")
    return spatial_factors, channel_factors, n_groups

  for i_n_groups in range(3, 5):
    factor_model = MatrixDecomposer(i_n_groups, decomposition_method)
    if decomposition_method == 'FactorAnalysis':
      _ = factor_model.fit(AM)
      spatial_factors = factor_model.transform(AM)
    else:
...
def map_neuron_attr_to_long(channel_factors, neuron_attrs, kept_channel_index):
  channel_factors_max_index = channel_factors.argmax(axis=0)
  decomposed_neuron_attrs = np.zeros((channel_factors.shape[0],) + neuron_attrs.shape, dtype=np.float32)
  short_index = []
  long_index = []
  ...
  for channel_factors_i in range(n_groups):
    ...
    decomposed_neuron_attrs[channel_factors_i][..., map_short_to_long_idx] = \
      neuron_attrs[..., map_short_to_long_idx]
  return channel_factors_max_index, decomposed_neuron_attrs, short_index, long_index, n_groups
...
def get_sort_groups_with_shap_scores(channel_shap):
  ...
  # channel_shap_max_index_temp = channel_shap.argmax(axis=0)
  # channel_shap_max_index_temp[np.squeeze(np.argwhere(np.sum(channel_shap, axis=0) == 0))] = 74
  # channel_shap_unique_temp, channel_shap_counts_temp = \
  #   np.unique(channel_shap_max_index_temp, return_counts=True)
  # every_group_attr = np.divide(every_group_attr,
  #                              channel_shap_counts_temp[:n_groups])
